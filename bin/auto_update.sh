...
# 更新指数行情数据
java -cp $collect_jar com.abiao.data.action.update.IndexDailyDataAutoUpdateAction ${index_daily_data}
java -cp $collect_jar com.abiao.data.action.commit.CommonCommitter --source index_daily_data --input ${index_daily_data}


#计算估值
java -cp $computer_jar com.abiao.data.valuation.IndexValuationAction --commitMode local --localOutput ${index_valuation_data}
java -cp $collect_jar com.abiao.data.action.commit.CommonCommitter --source index_valuation --input ${index_valuation_data}
